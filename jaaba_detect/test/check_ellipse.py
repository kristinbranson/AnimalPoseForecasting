"""Compare perframe_ellipse.py against MATLAB reference (ellipse_gt.mat)."""
import sys, os
import numpy as np
...
def main(gtfile, trxfile):
    ...
    ell = perframe_ellipse.EllipseFeatures(traj)
    ...
    for fi, fn in enumerate(names):
        ...
        for i in range(traj.nflies):
            ...
            if not np.array_equal(ng, npn):
                print(f"  NAN {fn} fly{i}: {np.sum(ng != npn)} differ"); nfail += 1
            both = ~ng & ~npn
            if both.any():
                mx = float(np.abs(g[both] - p[both]).max())
                if mx > worst: worst, worst_where = mx, f"{fn} fly{i}"
                if mx > 1e-6: print(f"  MISMATCH {fn} fly{i}: {mx:.3e}"); nfail += 1
