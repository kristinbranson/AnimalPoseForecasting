...
# %%
# Imports and environment setup: load packages, configure logging, detect notebook vs. script context.
# %load_ext autoreload
# %autoreload 2
# #%matplotlib widget
# %matplotlib inline
...
# %%
# Resolve paths: locate the female-specific config file and set up the output figure directory.
...
# all outputs go under the working directory, never into the source tree
outrootdir = os.getcwd()
# assert not os.path.abspath(outrootdir).startswith(os.path.abspath(flyllmdir)), \
#     f'Working directory {outrootdir} is inside the source tree {flyllmdir}; cd elsewhere before running.'
...
# %%
# Generate a timestamp string used to uniquely label output files for this run.
timestamp = time.strftime('%Y%m%dT%H%M%S')
LOG.info(f'Timestamp for this run: {timestamp}')

# %%
# Optional profiling block (disabled): profile the binning optimizer and print the top time-consuming calls.
if False:
    import cProfile
    import pstats
...
# %%
# Load the config (overriding to use all courtship data) and build the training dataset,
...
# %%
# Build the output filename stem from the model type string and timestamp.
...
# %%
# Sanity-check the optimal binning code on synthetic data before running on real data.
debug_optimal_binning.debug_synthetic()
#debug_optimal_binning.debug_zscored_velocity()

# %%
# Plot the tail percentiles of each velocity feature on log-log axes (positive and negative sides separately),
# to visually inspect the data distribution before choosing bin edges.
...
# %%
# Core optimization loop: for each velocity feature to be discretized, extract the data,
# subsample into sequences, determine outlier thresholds and minimum bin widths, fix the
# boundary edges, then run the bin-edge optimizer to find the K edges that maximize log-likelihood.
config = init_config(configfile=configfile,overrideconfig=overrideconfig)['config']
...
# %%
# Plot the optimized bins for each feature: bin widths vs. index, bin widths vs. center,
# edge positions, and the bin-to-bin transition probability matrix. Saves PNG and PDF figures.
for idx,featidx in enumerate(config['discreteidx']):
...
# %%
# Save the optimized bin info (edges, transition probabilities, log-likelihood) for all features
# to a compressed .npz file, annotated with feature indices and names.
# a relative savedir is resolved against the working directory, not the source tree
# savedir = config['savedir']
# if not os.path.isabs(savedir):
#     savedir = os.path.join(outrootdir,savedir)
savedir = outrootdir
savefile = os.path.join(savedir,f'{savefilestr}_bininfo.npz')
...
# %%
# Print the optimized bin edges for each feature in a format ready to copy-paste into config files.
for i,featidx in enumerate(config['discreteidx']):
