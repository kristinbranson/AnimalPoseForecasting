...
def simulate_diffusion(
    ...
    curr_frame = burn_in
    model_input[0, :curr_frame, :] = torch.from_numpy(gt_input[:curr_frame])
    d_labels = gt_chunk['labels'].shape[-1]
    model_input[:, :, -d_labels:] = torch.randn(1, track_len, d_labels)
    model_input = model_input.to(device)
    ...
    if hasattr(model, 'noise_scheduler'):
        ...
        LOG.warning('Model does not have a noise_scheduler, creating one with default values')
    ...
    for curr_frame in tqdm(range(burn_in, track_len)):
        frame0 = 0
        if max_contextl is not None:
            frame0 = max(0, curr_frame - max_contextl)
        ...
        # Apply model to previous frames
        input = model_input[:, frame0:curr_frame, :].clone()
        with torch.no_grad():
            for t in noise_scheduler.timesteps:
                t_noise = t * torch.ones((1,)).long().to(device)
                pred = model.output(input, t_noise=t_noise, mask=mask, is_causal=True)
                input[:, :, -d_labels:] = noise_scheduler.step(
                    model_output=pred['continuous'],
                    timestep=t_noise,
                    sample=input[:, :, -d_labels:],
                ).prev_sample
        ...
        inputs_proc = apply_opers_from_data(dataset.inputs, inputs)
        curr_in = np.concatenate(list(inputs_proc.values()), axis=-1)
        model_input[:, curr_frame, :-d_labels] = torch.from_numpy(curr_in.astype(np.float32)).to(device)[:, :, :-d_labels]
