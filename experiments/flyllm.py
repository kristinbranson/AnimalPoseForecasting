...
class Pose(Operation):
    ...
    def apply(self, Xkp: np.ndarray, scale_perfly: np.ndarray | None = None, flyid: np.ndarray | None = None, isdata: np.ndarray | None = None) -> np.ndarray:
        ...
        Args:
            Xkp: (x, y) pixel position of the agents, (n_agents,  n_frames, 2, n_keypoints) float array or (n_frames, 2, n_keypoints) float array
...
def make_dataset(
    ...
    if ref_dataset is not None:
        ...
        if discreteidx is None:
            pred_velocity = zscored_velocity
        else:
            ...
            pred_velocity = Fusion([Discretize(**bin_config), Identity()], indices_per_op)(zscored_velocity)
